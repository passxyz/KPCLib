...
namespace KeePassLib.Serialization
{
	...
	public sealed partial class KdbxFile
	{
        ...
        /// <summary>
        /// System.Drawing.ColorTranslator.FromHtml is missing in .NET 6. Copy from the source code temporarily.
			...
			switch(ctx)
			{
				...
				case KdbContext.Meta:
					...
					else if(xr.Name == ElemDbColor)
					{
						...
						if(!string.IsNullOrEmpty(strColor))
							m_pwDatabase.Color = FromHtml(strColor);
					}
				...
				case KdbContext.Entry:
					...
					else if(xr.Name == ElemFgColor)
					{
						string strColor = ReadString(xr);
						if(!string.IsNullOrEmpty(strColor))
							m_ctxEntry.ForegroundColor = FromHtml(strColor);
					}
					else if(xr.Name == ElemBgColor)
					{
						string strColor = ReadString(xr);
						if(!string.IsNullOrEmpty(strColor))
							m_ctxEntry.BackgroundColor = FromHtml(strColor);
					}
